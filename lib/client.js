...
class ConfigClient {
  ...
  connect(){
    return function connectConfigServer(callback){
      ...
      this.socket.on(Const.EVENT_SERVICE, (args)=>{
        var id = args.id;
        var service = args.service;
        var sid = accessedServices[service];
        if ( service ) {
          if ( sid ){
            this.socket.emit(Const.EVENT_SERVICE, {id: id, sid: sid});
          }
        }
        else {
          var services = [];
          for ( var service in accessedServices ) {
            services.push(service);
          }
          this.socket.emit(Const.EVENT_SERVICE, {id: id, services: services});
        }
      });
  ...
  get(service){
    accessedServices[service] = this.sid;
